        ...
        <div class="border-t border-subtle pt-6">
             @auth('student')
                 <form action="{{ route('student.logout') }}" method="POST">
                     @csrf
                     <button type="submit" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-secondary hover:text-error hover:bg-error/10 transition-colors w-full cursor-pointer">
                         @svg('heroicon-o-arrow-left-on-rectangle', 'w-5 h-5')
                         <span class="font-medium">Logout</span>
                     </button>
                 </form>
             @else
                 <a href="{{ route('student.login') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-secondary hover:text-accent hover:bg-accent/10 transition-colors">
                     @svg('heroicon-o-user-plus', 'w-5 h-5')
